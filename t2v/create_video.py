...
def main():
    ...
    with torch.no_grad():
        uncond_embed = text2img.text_encoder(uncond_input.input_ids.to(device))[0]
    ...
    for i, lines in enumerate(whispers):
        ...
        prompt1_tok = text2img.tokenizer(prompt_1+added_prompt , padding='max_length', max_length=60, truncation=True,
                                      return_tensors="pt")
        with torch.no_grad():
            prompt1_embed = text2img.text_encoder(prompt1_tok.input_ids.to(device))[0]

        prompt2_tok = text2img.tokenizer(prompt_2+added_prompt, padding='max_length', max_length=60, truncation=True,
                                      return_tensors="pt")
        with torch.no_grad():
            prompt2_embed = text2img.text_encoder(prompt2_tok.input_ids.to(device))[0]


        if i == 0:
            params = {"t0": 44, "t1": 48, "motion_field_strength_x": 5, "motion_field_strength_y": 5,
                      "smooth_bg": False,
                      "video_length": math.ceil(0.9 * num_steps), "chunk_size": args.chunk_size,
