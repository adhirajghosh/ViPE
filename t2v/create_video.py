...
def parse_args():
    ...
    parser.add_argument("--dataset_dir", type=str, default='/graphics/scratch2/staff/Hassan/genius_crawl/lyrics_to_prompts_v2.0.csv',help='path to the training data')
    parser.add_argument("--checkpoint", type=str, default='./models/vipe/', help="path to save the model")
    ...
    parser.add_argument("--gap_prompt", type=str, default='song: skyfall, genre: orchestral pop, artist: Adele')
    parser.add_argument("--t2v_frames", type=str, help='Number of frames in one t2v run-through', default=8)
    ...
    parser.add_argument('--url', help='youtube url if song needs to be downloaded', type=str, default='https://www.youtube.com/watch?v=DeumyOzKqgI&ab_channel=AdeleVEVO')
...
def main():
    ...
    text2vid = Model(device=device, dtype=torch.float16)
    ...
    frame_index = 0
    uncond_input = text2img.tokenizer(negative_prompt, padding='max_length', max_length=60, truncation=True, return_tensors="pt")
    with torch.no_grad():
    ...
    for i, lines in enumerate(whispers):
        ...
        if i == 0:
            params = {"t0": 44, "t1": 48, "motion_field_strength_x": 5, "motion_field_strength_y": 5,
                      "smooth_bg": False,
                      "video_length": math.ceil(0.9 * num_steps), "chunk_size": args.t2v_frames,
                      "resolution": args.img_size}

            frame_index, _ = text2vid.process_text2video(prompt_1+added_prompt, model_name=model_id, fps=args.fps, path=args.outdir,
            ...
            params = {"t0":44, "t1": 48, "motion_field_strength_x": 5, "motion_field_strength_y": 5,"resolution": args.img_size,
                      "smooth_bg": False, "video_length": math.ceil(num_steps-bleed_over- 0.1*num_steps), "chunk_size": args.t2v_frames}

            frame_index, _ = text2vid.process_text2video(prompt_1 + added_prompt, model_name=model_id, fps=args.fps,
                                                      path=args.outdir,
