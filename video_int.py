...
def parse_args():
    ...
    parser.add_argument('--device', help='gpu', type=str, default='1')
    parser.add_argument('--url', help='youtube url if song needs to be downloaded', type=str, default='https://www.youtube.com/watch?v=GtEvysh1654&ab_channel=7clouds')
    parser.add_argument('--songdir', help='Where songs are kept', type=str, default='./mp3/')
    parser.add_argument('--outdir', help='Where results are kept', type=str, default='./results/vids/Thunder/2/')
...
def main():
    ...
    for i, lines in enumerate(whispers):
        ...
        #
        #     init_image_1 = image
        #     init_a2 = img2emb(img2img, init_image_1, seed, device)
        #
        #     init_image_2 = text2img(prompt_2 + added_prompt, height=args.img_size, width=args.img_size,
        #                             negative_prompt=negative_prompt).images[0]
        #     init_b2 = img2emb(text2img, init_image_2, seed, device)
        #     init_a3 = init_a+init_a2
        #     init_b3 = init_b+init_b2
        ...
        #         else:
        #             cond_embedding = slerp(float(t), prompt1_embed, prompt2_embed)
        #         init = slerp(float(t), init_a3.detach(), init_b3.detach())
        #
        #         with autocast("cuda"):
        #             im = diffuse(text2img, cond_embedding, init, uncond_embed, args.inf_steps, args.guidance_scale,
        #                          args.eta)
        #
        #         image = Image.fromarray(im)
        #         outpath = os.path.join(args.outdir, 'frame%06d.jpg' % frame_index)
        ...
        #     count = 0
        #
        #     while count<(num_steps-bleed_over- 0.1*num_steps):
        #         print("generating... ", frame_index)
        #         image = img2img(prompt = prompt_1+added_prompt, image = init_image_1, negative_prompt = negative_prompt,
        #                         strength=args.strength, guidance_scale=args.guidance_scale, generator=generator).images[0]
        #         outpath = os.path.join(args.outdir, 'frame%06d.jpg' % frame_index)
        ...
        #     init_image_1 = init_image_2
        #     init_a = init_b
        #     bleed_over = math.ceil(0.2*num_steps)
        #     # init_image_1 = im
        if i == 0:
            for j, t in enumerate(np.linspace(0, 1, math.ceil(1.1*num_steps))):
                print("generating... ", frame_index)
                if j < math.ceil(0.9*num_steps):
                    flag = False
                else:
                    flag = True
                cond_embedding = lerp(float(t), prompt1_embed, prompt2_embed, flag)
                init = lerp(float(t), init_a.detach(), init_b.detach(), flag)
            ...
            for j, t in enumerate(np.linspace(0, 1, num_steps)):
                ...
                if j< num_steps-bleed_over- math.ceil(0.1*num_steps):
                    flag = False
                else:
                    flag = True
                cond_embedding = lerp(float(t), prompt1_embed, prompt2_embed, flag)
                init = lerp(float(t), init_a, init_b, flag)
            ...
            for j, t in enumerate(np.linspace(0, 1, num_steps-bleed_over)):
                print("generating... ", frame_index)

                if j< num_steps-bleed_over- math.ceil(0.2*num_steps):
                    flag = False
                else:
                    flag = True
                cond_embedding = lerp(float(t), prompt1_embed, prompt2_embed, flag)
